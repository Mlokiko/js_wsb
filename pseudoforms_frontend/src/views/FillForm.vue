...
export default {
  ...
  try {
    const payload = {
      respondent_id: "user_123", // lub null, jeśli użytkownik nie jest zalogowany
      responses: this.form.questions.map((q, i) => ({
        question_id: q.id,
        response_text: Array.isArray(this.responses[i]) ? this.responses[i].join(', ') : this.responses[i] // Dla pytań wielokrotnego wyboru
      }))
    };
